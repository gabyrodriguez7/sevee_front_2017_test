      ...
      <div class="content-descarga">
        <v-card>
          ...
          <div lass=" justify-center" v-if="vuelta == 1">
            <v-fab-transition>
              <v-btn active-class="deep-purple" class="boton-descarga" @click="descargarZIP" color="#12A2C2" dark absolute
                stacked>
                <div>{{ vuelta }}. DATOS COMPLETOS ELECCIONES 2013</div>
...
export default {
  ...
  methods: {
    GetData(vuelta, nac) {
      ...
      const url = `Tabla2013/${nac}${vuelta}v.json`;
      console.log(url);
    ...
    async descargarZIP() {
      ...
      const url = 'DatosCompletos2013/2013_Elecciones.zip';
      ...
      try {
        ...
        if (response.ok) {
          ...
          const nombreZIP = ` ${this.vuelta} . DATOS COMPLETOS ELECCIONES 2013.zip`;
